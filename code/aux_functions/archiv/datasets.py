...
# # stuff to debug SYNTHETIC2
# import pandas as pd
# path_data = "D:/07_IsThereEA_v2/data/bucket2/nodraw"
# catch_id = 0
...
class SYNTHETIC(BaseDataset):
    ...
    def __init__(self, 
        ...
        super(SYNTHETIC, self).__init__(dynamic_input = dynamic_input,
                                        target = target, 
    ...
    def _read_attributes(self) -> pd.DataFrame:
        ...
        # read attributes
        if self.noise != '':
            path_attributes = Path(self.path_data) / ("staticdata_"+self.noise+".csv")
        if self.noise == '':
            path_attributes = Path(self.path_data) / "staticdata.csv"
        df = pd.read_csv(path_attributes, index_col=0)
    ...
    def _read_data(self, catch_id: str) -> pd.DataFrame:
        ...
        # read forcings
        df_forcings = self._read_forcings()
                        
        # read target data
        path = Path(self.path_data) / "modelruns" / f"{catch_id}.csv"
        df = pd.read_csv(path)
        df = df.set_index("date")
        df.index = pd.to_datetime(df.index, format="%Y-%m-%d")
        
        # merge target data and forcing data
        df = pd.merge(df_forcings, df, on='date', how='inner')
        
        return df
    
    def _read_forcings(self) -> pd.DataFrame:
        
        forcing_path = Path(self.path_forcing)
        df = pd.read_csv(forcing_path,index_col=0)
        df.index = pd.to_datetime(df.index, format="%Y-%m-%d")
