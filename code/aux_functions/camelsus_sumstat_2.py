...
class CAMELS_US(BaseDataset):
    ...
    def _read_attributes(self)-> pd.DataFrame:
        ...
        path_attributes = Path(self.path_data) / "basin_mean_forcing" / 'all_sumstat_2.csv'

        # Read the attributes files
        df = pd.read_csv(path_attributes, sep=';', header=0, dtype={'gauge_id': str})
        df["gauge_id"] = [str(item).zfill(8) for item in df["gauge_id"]]
    ...
    def _load_forcing(self, catch_id: str, forcing : str) -> Tuple[pd.DataFrame, int]:
        ...
        file_path = list(forcing_path.glob(f'**/{catch_id}.csv'))
